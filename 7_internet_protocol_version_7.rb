...
input = (ARGV.delete("-t") ? TEST : DATA).each_line.map(&:strip).map do |line|
  toks = line.split(/[\[\]]/)
  outs, ins = toks.partition.each.with_index { |_, i| i.even? }
  [outs, ins]
end

def abba?(str)
  pair_pos = str.chars.each_cons(2).to_a.map.with_index { |pair, i| pair.uniq.count == 1 ? i : -1 }.select(&:positive?)
  pair_pos.map do |pos|
    return false if pos < 1 || pos > str.length - 3
    outers = [str[pos - 1], str[pos + 2]]
    return false if outers[0] != outers[1]
    return false if outers[0] == str[pos]
    true
  end.any?
end

input.map do |address|
  address[1].map(&method(:abba?)).none? && address[0].map(&method(:abba?)).any?
end.count(true).tap(&method(:puts))
