...
input = (ARGV.delete("-t") ? TEST : DATA).each_line.map do |line|
  line.strip!
  *chars, idcheck = line.split("-")
  id, check = idcheck.chars.tap(&:pop).join.split("[")
  [chars, id, check]
end
...
input.map do |line|
  line[0]
    ...
    .chars
    .group_by(&:itself)
    .tap { |a| a.each { |k, v| a[k] = v.count } }
    .group_by { |k, v| v }
    .tap { |h| h.each { |k, v| h[k] = v.map { |c| c[0] }.sort.join } }
    .sort
    .reverse
    .map { |k, v| v }
    .join[0...5] == line[2] ? line[1].to_i : 0
end.sum.tap(&method(:puts))

input.map do |line|
  name = line[0].map do |word|
    word.each_byte.to_a.map { |b| ((b - 97 + line[1].to_i) % 26) + 97 }.map(&:chr).join
  end.join(" ")
  puts "#{name} -- #{line[1]}"
end
