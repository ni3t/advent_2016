...
bot; the rest of the instructions indicate what a given bot should do with its 
...
and output bin 2 contains a value-3 microchip. In this configuration, bot number 2 is 
...
input = (ARGV.delete("-t") ? TEST : DATA).each_line.map(&:strip).map do |line|
  val, bot1, gives, low, to, bot, bot2, andd, high, toto, botoutput, bot3 = line.split(" ")
end

Bot = Struct.new(:holding, :lowgive, :highgive, :lowtype, :hightype)
bots = Array.new(225) { Bot.new([], nil, nil) }
outputs = Array.new(30) { [] }
...
input.map do |instr|
  if instr[0] == "value"
    ...
    high = instr[11].to_i
    type2 = instr[10]
    bots[bot1].lowgive = low
    bots[bot1].lowtype = type1
    bots[bot1].highgive = high
    bots[bot1].hightype = type2
  end
end

until bots.map(&:holding).map(&:count).count(2) == 0
  holdingbot = bots.select { |b| b.holding.count == 2 }.first
  if holdingbot.hightype == "output"
    outputs[holdingbot.highgive] << holdingbot.holding.max
  else
    bots[holdingbot.highgive].holding << holdingbot.holding.max
  end
  if holdingbot.lowtype == "output"
    outputs[holdingbot.lowgive] << holdingbot.holding.min
  else
    bots[holdingbot.lowgive].holding << holdingbot.holding.min
  end
  # pp bots.first(3).map(&:holding)
  # pp outputs.first(5)
  # pp "-" * 25
  if holdingbot.holding.sort == [17, 61]
    puts bots.index(holdingbot)
  end
  holdingbot.holding = []
end
